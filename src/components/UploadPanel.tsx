...
export default function UploadPanel(props: UploadPanelProps) {
  const { handleImageUpload, handleAudioUpload, audioName, aspectRatio, formatTime, audioDuration, currentTime, images, currentImageIndex, isPlaying, chorusBoost, activePreset, audioMood, imageMotions, onSelectImage, backgroundMode, setBackgroundMode, audioSourceMode, setAudioSourceMode } = props;
  ...
  return (
    <div className="h-full rounded-2xl border border-fuchsia-500/30 bg-zinc-950/90 p-4 overflow-y-auto space-y-4">
      ...
      <div className="rounded-xl border border-zinc-700 bg-zinc-900/80 p-3 text-xs space-y-2">
        ...
        <p className="text-[10px] text-zinc-400">動画背景は現在無効化中です。</p>
      </div>
      ...
      <div className="rounded-xl border border-zinc-700 bg-zinc-900/80 p-3 text-xs space-y-1">
        ...
        <div className="text-yellow-300">背景：{backgroundMode === "none" ? "背景なし" : "画像背景"}</div>
        <div className="text-yellow-300">音声：{audioSourceMode === "bgm" ? "BGM" : "無音"}</div>
