...
export default function PreviewStage(props: PreviewStageProps) {
  const { previewSizeClass, chorusBoost, selectedImage, isPlaying, isRecording, getMotionStyle, getMotionAmplitude, showStatusOverlay = true } = props;
  ...
  return (
    <div className="relative flex w-full items-center justify-center">
      ...
      <div className={`${previewSizeClass} mt-2 md:mt-10 bg-zinc-900 border-2 border-cyan-300/80 rounded-xl md:rounded-2xl relative overflow-hidden flex items-center justify-center transition-all duration-150 shadow-[0_0_30px_rgba(34,211,238,0.18)] md:shadow-[0_0_40px_rgba(34,211,238,0.2)] ${chorusBoost ? "shadow-[0_0_55px_#ec4899] scale-[1.015] md:shadow-[0_0_70px_#ec4899] md:scale-[1.02]" : ""}`}>
        {selectedImage ? (
          <img src={selectedImage} alt="" className={`w-full h-full object-cover ${chorusBoost ? "contrast-125 saturate-150" : ""}`} style={imageStyle} />
        ) : (
